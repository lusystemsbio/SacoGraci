...
# tempM: temperature of metropolis algorithm
# (output): 
SacoGraci_optimization_MH<-function(network_top, data, clusterRef, cenMedRef, cutOffM, gene_list,inTopsM ,output = "Results", 
                                   numbThr = 40, nSim = 20, nRepeat= 5, nIter = 1400, modelsCGr = 10000, tempM = 60){
  require(doParallel)
...
#  clusterExport(cl, varlist=c('dataRow', 'clusterRef','cenMedRef', 'cutOffM', 'gene_list', 'flnV'))
  ...
  for(itn in 1:nIter){
    ...
    for(i in 1:nSim){  
      ...
      if (rnUn< exp((last_scM[i,1]-new_scM[i,1])/tempM)){
