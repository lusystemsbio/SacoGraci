...
opt_SA_multi<-function(network_top, data, clusterRef, cenMedRef, cutOffM, gene_list, inTopsM, 
  ...
  while(current_temp>1){
    ...
    while(numb_iter<iter_per_temp){
      ...
      if(nUpd>0){
        write.table(t(c(numb_iter,indNew)),file=fileOutStuff,append=TRUE, sep="\t", row.names=F, col.names=F)
        ...
        if(realSim>1){
          xouts<-cbind(rep(numb_iter,realSim),cbind(cbind(redM,scSampV[(nSamp+1):(nSamp+realSim)]),thrownOut[(nSamp+1):(nSamp+realSim)]))
        }
        if(realSim==1){
          xouts<-c(numb_iter,as.vector(redM),scSampV[nSamp+1],thrownOut[nSamp+1])
          xouts<-matrix(xouts,nrow=1)
        }
      ...
      for(i in 1:nSim){  
        ...
        if (rnUn< exp((last_scM[i,1]-new_scM[i,1])/current_temp)){
